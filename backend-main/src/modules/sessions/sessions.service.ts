...
export class SessionsService {
  constructor(@InjectModel(Sessions.name) private sessionsModel: Model<SessionsDocument>) { }
  ...
  async update(id: string, updateSessionsDto: UpdateSessionsDto): Promise<Sessions> {
    const updatedSessions = await this.sessionsModel.findByIdAndUpdate(
      id,
      updateSessionsDto,
  ...
  async remove(id: string): Promise<void> {
    ...
    if (!sessionsToDelete) throw new NotFoundException(`Client avec id=${id} non trouvé`);
  }
}
